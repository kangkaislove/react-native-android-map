...
public class BaiduMapViewManager extends ViewGroupManager<MapView> {
    ...
    private HashMap<String, List<Marker>> mChargeMarkersMap = new HashMap<>();
    ...
    public void setCurrentLocation(MapView mapView, ReadableMap position) {
        if(position != null) {
            String key = "marker_" + mapView.getId();
            Marker marker = mMarkerMap.get(key);
            if(marker != null) {
                MarkerUtil.updateLocation(marker, position);
            }
            else {
                marker = MarkerUtil.showLocation(mapView, position,mReactContext);
    ...
    @ReactProp(name="carMarkers")
    public void setCarMarkers(MapView mapView, ReadableArray options) {
    ...
    @ReactProp(name="chargeMarkers")
    public void setChargeMarkers(MapView mapView, ReadableArray options) {
        String key = "chargeMarkers_" + mapView.getId();
        List<Marker> markers = mChargeMarkersMap.get(key);
        if(markers == null) {
            markers = new ArrayList<>();
        }
        for (int i = 0; i < options.size(); i++) {
            ...
            if(markers.size() > i + 1 && markers.get(i) != null) {
                ...
                markers.add(i, MarkerUtil.addChargeMarker(mapView, option,mReactContext));
            }
        }
        if(options.size() < markers.size()) {
            int start = markers.size() - 1;
        ...
        mChargeMarkersMap.put(key, markers);
    }
